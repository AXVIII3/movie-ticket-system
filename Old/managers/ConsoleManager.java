...
public class ConsoleManager
{
    ...
    private static void SelectMovieMessage()
    {
        ...
        {
            movie = BookingManager.movies[choice - 1];
    ...
    {
        if (movie.formattedDates.length > 1)
        {
            System.out.println("Which date do you want to book?");
            int i = 1;
            for (; i <= movie.formattedDates.length; i++)
                    System.out.println(i + ") " + movie.formattedDates[i - 1]);
        ...
        ConfigureNumberOfSeats();
    }

    private static void ConfigureNumberOfSeats()
    {
        ...
        ChooseSeats(numberOfSeats);
    }

    private static void ChooseSeats(int numberOfSeats)
    {
        int[] unavailableSeats = BookingManager.GetUnavailableSeats(movie);
        ...
        for (int i = 1; i <= numberOfSeats; i++)
        {
            ...
            for (int bookedSeat : BookingManager.GetUnavailableSeats(movie))
        ...
        FinalScreen(seats, movie.CalculateCost(seats.length));
    }

    private static void FinalScreen(int[] seats, int cost)
    {
        System.out.println("\nBOOKING DETAILS:");
        System.out.println("Movie Name: " + movie.name);
        System.out.println("Duration: " + movie.duration);
        System.out.println("Date and Time: on" + " at " + movie.time);
